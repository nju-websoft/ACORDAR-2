...
class LazyBatcher():
    def __init__(self, args, rank=0, nranks=1, isTrain=True):
        ...
        self.valid_triples = self._load_triples(args.valid_triples, rank, nranks)
        self.valid_queries = self._load_queries(args.valid_queries)
    ...
    def __next__(self):
        if self.is_train:
            ...
            offset, endpos = self.position, min(self.position + 1, len(self.valid_triples))
            self.position = endpos
            # print(offset,endpos)

            if offset + 1 > len(self.valid_triples):
                raise StopIteration
            ...
            for position in range(offset, endpos):
                # print(position)
                query, pos, neg = self.valid_triples[position]
                query, pos, neg = self.valid_queries[query], self.collection[pos], self.collection[neg]
