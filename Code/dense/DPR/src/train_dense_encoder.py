...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        ...
        if cfg.local_rank in [-1, 0]:
            ...
            with open(os.path.join(cfg.output_dir, 'best_cp_name'), 'w+') as fp:
                fp.write(f"{self.best_cp_name}\t{self.best_validation_result}")
...
def main(cfg: DictConfig):
    cfg.train.batch_size=cfg.my_batch_size
    cfg.train.learning_rate=cfg.my_learning_rate
    if cfg.train.gradient_accumulation_steps < 1:
        raise ValueError(
